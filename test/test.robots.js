...
lab.experiment('hapi-robots', () => {
  ...
  lab.test('supports sitemap as string', async() => {
    await server.register({
      plugin: robotModule,
      options: {
        verbose: true,
  ...
  lab.test('supports sitemap as array of strings', async() => {
    await server.register({
      ...
      options: {
        verbose: true,
        sitemap: ['http://somewhere.com/sitemap.xml', 'http://somewhere.com/sitemap-categories.xml']
      }
    });
    const response = await server.inject({
      method: 'get',
    ...
    Code.expect(response.statusCode).to.equal(200);
    Code.expect(response.payload.includes('Sitemap: http://somewhere.com/sitemap.xml')).to.equal(true);
    Code.expect(response.payload.includes('Sitemap: http://somewhere.com/sitemap-categories.xml')).to.equal(true);
  });
});
